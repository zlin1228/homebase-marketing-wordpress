<?php
/* Template name: Flexible content 2
 * Template version: V1
 * Created By: Baljinder S
 * Date: Jan 31, 2020
 *  */
get_header();
?>
...
/*
*
* FLEXIBLE CONTENT 2 for New modules
*
* Developed By Baljinder
*
*/
					...
					<div class="row reviews">
									<?php if ( have_rows('reviews') ) :
											while ( have_rows('reviews') ) : the_row();
													...
													echo '          <div class="score"><img class="lazyload" data-src="' . get_sub_field('score') . '" /></div>';
													echo '          <div class="company"><img class="lazyload"  data-src="' . get_sub_field('company_logo') . '" /></div>';
...
<?php if ($list_meta) : ?><p class="list-meta"><?php echo $list_meta; ?></p><?php endif; ?>
      ...
      elseif ( get_row_layout() == 'flex_simple_button' ) : ?>

					<?php if (!get_sub_field('hide_widget')) : ?>

					<?php $scroll_anchor = get_sub_field('scroll_anchor'); ?>

					<div class="simplebtn-section" <?php echo ($scroll_anchor) ? 'data-anchor="#' . $scroll_anchor . '"' : ''; ?>>
						...
						<div class="row">
							<div class="columns large-6 medium-8 large-offset-3 medium-offset-2">
									<div class="simple-btn text-center">
										<a href="<?php echo $button_link; ?>">
											<?php echo $button_text; ?>
										</a>
									</div>
		...
		elseif ( get_row_layout() == 'flex_fixed_top_button' ) : ?>
		...
		// elseif ((get_row_layout() == 'flex_app_download') && wp_is_mobile())
		elseif ((get_row_layout() == 'flex_app_download') && wp_is_mobile()): ?>
			<?php if (!get_sub_field('hide_widget')) : ?>
				<?php
					$app_icon          		= get_sub_field('app_icon');
					$app_name            	= get_sub_field('app_name');
					$app_title        		= get_sub_field('app_title');
					$app_price         		= get_sub_field('app_price');
					$app_stroe_url        = get_sub_field('app_stroe_url');
					$google_player_url    = get_sub_field('google_player_url');
					$app_score 						= get_sub_field('app_score');
				...
				<div class="app-view">
					<div class="row">
						<div class="cancel-icon">
							<a href="#" class="button cancel-btn">X</a>
						</div>
						<div class="col-app-icon">
							<img class="lazyload" alt="App Info" data-src="<?php echo $app_icon; ?>">
						</div>
						<div class="col-app-desc">
							<h3><?php echo ($app_name); ?></h3>
							<p><?php echo ($app_title); ?></p>
							<div class="sub-app-score">
								<img class="lazyload" alt="App Info" data-src="<?php echo $app_score; ?>">
							</div>
							<p><?php echo ($app_price); ?></p>
						</div>
						<div class="col-app-link">
							<!-- <a href="<?php echo ($app_stroe_url); ?>" class="button"><?php echo ($button_label); ?></a> -->
							<?php if (preg_match( '/iPad|iPod|iPhone/', $_SERVER['HTTP_USER_AGENT'] )) : ?>
								<a href="<?php echo ($app_stroe_url); ?>" class="button"><?php echo ($button_label); ?></a>
							<?php elseif (preg_match( '/Android/', $_SERVER['HTTP_USER_AGENT'])) : ?>
								<a href="<?php echo ($google_player_url); ?>" class="button"><?php echo ($button_label); ?></a>
							<?php endif; ?>
						</div>
					</div>
				</div>
...
<script type="text/javascript">
		window.addEventListener( 'load', function() {
			setTimeout(function(){
				if(checkCookie())
					jQuery(".app-view").fadeIn("slow");
			}, 2000);

			jQuery( "a.cancel-btn" ).click(function(e) {
				e.preventDefault();
				setCookie("appView", "notshow", 7);
				jQuery(".app-view").fadeOut("slow");
			});
		});
		...
		function checkCookie() {
			var showAppView=getCookie("appView");
			if (showAppView != "") {
				return false;
			} else {
				return true;
