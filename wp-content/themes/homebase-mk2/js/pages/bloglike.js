jQuery(document).ready(function($) {
  ...
  $('.section-faqs').find('h2').each(function(hnum) {
    var item = $(this);
    var inner_text = $(this).text();
    var classlist = $(this).attr('class');

    if (classlist != null) {
      $(this).replaceWith('<h2 id="toc-99" class="' + classlist + '">' + inner_text + "</h2>");
    }
    else {
      $(this).replaceWith('<h2 id="toc-99">' + inner_text + "</h2>");
    }
    

    var li = $('<li/>');
    var a = $('<a/>', {text: item.text(), href: '#toc-99', title: item.text()});
    a.appendTo(li);
    $('#section-table-of-contents .table-of-contents ul').append(li);
  });
});
